...
from main.enums import MediaStatus, VoteStatus, MediaActions, ParticipantStatus
...
def add_media(**kwargs):
    ...
    if User.get_user_by_email(user.email) is not None: 
        ...
        if participant.status == ParticipantStatus.IN: # check whether user is in the room or not to add media
